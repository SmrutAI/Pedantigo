	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			case *FormStringCondition:
				validator := pdcore.New[FormStringCondition]()
			...
			case *FormBoolCondition:
				validator := pdcore.New[FormBoolCondition]()
				err := validator.Validate(form)
				if tt.wantErr {
					require.Error(t, err, "expected validation error")
					var ve *pdcore.ValidationError
			...
			case *FormIntCondition:
				validator := pdcore.New[FormIntCondition]()
				err := validator.Validate(form)
				if tt.wantErr {
					require.Error(t, err, "expected validation error")
					var ve *pdcore.ValidationError
			...
			case *FormMultiple:
				validator := pdcore.New[FormMultiple]()
				err := validator.Validate(form)
				if tt.wantErr {
					require.Error(t, err, "expected validation error")
				} else {
...
func TestCrossFieldConstraints(t *testing.T) {
	t.Run("complex scenario - valid business account", func(t *testing.T) {
		...
		validator := pdcore.New[UserProfile]()
