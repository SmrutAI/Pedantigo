package constraints_test

import (
	"testing"
	"time"

	"github.com/stretchr/testify/assert"
	"github.com/stretchr/testify/require"

	. "github.com/SmrutAI/pedantigo/v2/validator"
)

// TestCrossFieldComparison tests all cross-field comparison constraints.
// This test covers eqfield, nefield, gtfield, gtefield, ltfield, ltefield.
func TestCrossFieldComparison_BasicOperators(t *testing.T) {
	tests := []struct {
		name      string
		testFunc  func(t *testing.T)
		expectErr bool
	}{
		// eqfield (field equals another field) tests
		{
			name: "eqfield: string equality valid",
			testFunc: func(t *testing.T) {
				type PasswordChange struct {
					Password string `validate:"required"`
					Confirm  string `validate:"required,eqfield=Password"`
				}

				vl := New[PasswordChange]()
				data := PasswordChange{
					Password: "secret123",
					Confirm:  "secret123",
				}

				err := vl.Validate(&data)
				assert.NoError(t, err)
			},
			expectErr: false,
		},
		{
			name: "eqfield: string equality invalid",
			testFunc: func(t *testing.T) {
				type PasswordChange struct {
					Password string `validate:"required"`
					Confirm  string `validate:"required,eqfield=Password"`
				}

				vl := New[PasswordChange]()
				data := PasswordChange{
					Password: "secret123",
					Confirm:  "different456",
				}

				err := vl.Validate(&data)
				require.Error(t, err)

				var ve *ValidationError
				require.ErrorAs(t, err, &ve)

				foundError := false
				for _, fieldErr := range ve.Errors {
					if fieldErr.Field == "Confirm" {
						foundError = true
					}
				}
				assert.True(t, foundError)
			},
			expectErr: true,
		},
		{
			name: "eqfield: email equality valid",
			testFunc: func(t *testing.T) {
				type SignUp struct {
					Email      string `validate:"required,email"`
					EmailCheck string `validate:"required,email,eqfield=Email"`
				}

				vl := New[SignUp]()
				data := SignUp{
					Email:      "user@example.com",
					EmailCheck: "user@example.com",
				}

				err := vl.Validate(&data)
				assert.NoError(t, err)
			},
			expectErr: false,
		},
		{
			name: "eqfield: int equality valid",
			testFunc: func(t *testing.T) {
				type TransactionVerify struct {
					Amount  int `validate:"gt=0"`
					Confirm int `validate:"gt=0,eqfield=Amount"`
				}

				vl := New[TransactionVerify]()
				data := TransactionVerify{
					Amount:  10000,
					Confirm: 10000,
				}

				err := vl.Validate(&data)
				assert.NoError(t, err)
			},
			expectErr: false,
		},
		{
			name: "eqfield: float equality valid",
			testFunc: func(t *testing.T) {
				type PriceMatch struct {
					ExpectedPrice float64 `validate:"gt=0"`
					ActualPrice   float64 `validate:"gt=0,eqfield=ExpectedPrice"`
				}

				vl := New[PriceMatch]()
				data := PriceMatch{
					ExpectedPrice: 99.99,
					ActualPrice:   99.99,
				}

				err := vl.Validate(&data)
				assert.NoError(t, err)
			},
			expectErr: false,
		},
		{
			name: "eqfield: zero values equal valid",
			testFunc: func(t *testing.T) {
				type ZeroComparison struct {
					Field1 int `validate:"eqfield=Field2"`
					Field2 int `validate:"eqfield=Field1"`
				}

				vl := New[ZeroComparison]()
				data := ZeroComparison{
					Field1: 0,
					Field2: 0,
				}

				err := vl.Validate(&data)
				assert.NoError(t, err)
			},
			expectErr: false,
		},

		// nefield (field not equal to another field) tests
		{
			name: "nefield: float inequality valid",
			testFunc: func(t *testing.T) {
				type DiscountCode struct {
					OriginalPrice   float64 `validate:"gt=0"`
					DiscountedPrice float64 `validate:"gt=0,nefield=OriginalPrice"`
				}

				vl := New[DiscountCode]()
				data := DiscountCode{
					OriginalPrice:   100.00,
					DiscountedPrice: 80.00,
				}

				err := vl.Validate(&data)
				assert.NoError(t, err)
			},
			expectErr: false,
		},
		{
			name: "nefield: float inequality invalid",
			testFunc: func(t *testing.T) {
				type DiscountCode struct {
					OriginalPrice   float64 `validate:"gt=0"`
					DiscountedPrice float64 `validate:"gt=0,nefield=OriginalPrice"`
				}

				vl := New[DiscountCode]()
				data := DiscountCode{
					OriginalPrice:   100.00,
					DiscountedPrice: 100.00,
				}

				err := vl.Validate(&data)
				require.Error(t, err)
			},
			expectErr: true,
		},
		{
			name: "nefield: string inequality valid",
			testFunc: func(t *testing.T) {
				type UniqueLogins struct {
					Username string `validate:"required,min=3"`
					Nickname string `validate:"required,min=1,nefield=Username"`
				}

				vl := New[UniqueLogins]()
				data := UniqueLogins{
					Username: "john_doe",
					Nickname: "Johnny",
				}

				err := vl.Validate(&data)
				assert.NoError(t, err)
			},
			expectErr: false,
		},
		{
			name: "nefield: zero values inequality invalid",
			testFunc: func(t *testing.T) {
				type UnequalCheck struct {
					Field1 int `validate:"nefield=Field2"`
					Field2 int `validate:"nefield=Field1"`
				}

				vl := New[UnequalCheck]()
				data := UnequalCheck{
					Field1: 0,
					Field2: 0,
				}

				err := vl.Validate(&data)
				require.Error(t, err)
			},
			expectErr: true,
		},
		{
			name: "nefield: zero values inequality valid",
			testFunc: func(t *testing.T) {
				type UnequalCheck struct {
					Field1 int `validate:"nefield=Field2"`
					Field2 int `validate:"nefield=Field1"`
				}

				vl := New[UnequalCheck]()
				data := UnequalCheck{
					Field1: 0,
					Field2: 5,
				}

				err := vl.Validate(&data)
				assert.NoError(t, err)
			},
			expectErr: false,
		},

		// gtfield (field greater than another field) tests
		{
			name: "gtfield: int greater than valid",
			testFunc: func(t *testing.T) {
				type DateRange struct {
					StartYear int `validate:"min=1900"`
					EndYear   int `validate:"min=1900,gtfield=StartYear"`
				}

				vl := New[DateRange]()
				data := DateRange{
					StartYear: 2000,
					EndYear:   2024,
				}

				err := vl.Validate(&data)
				assert.NoError(t, err)
			},
			expectErr: false,
		},
		{
			name: "gtfield: int greater than invalid equal",
			testFunc: func(t *testing.T) {
				type DateRange struct {
					StartYear int `validate:"min=1900"`
					EndYear   int `validate:"min=1900,gtfield=StartYear"`
				}

				vl := New[DateRange]()
				data := DateRange{
					StartYear: 2000,
					EndYear:   2000,
				}

				err := vl.Validate(&data)
				require.Error(t, err)
			},
			expectErr: true,
		},
		{
			name: "gtfield: float greater than valid",
			testFunc: func(t *testing.T) {
				type PriceComparison struct {
					MinPrice float64 `validate:"gt=0"`
					MaxPrice float64 `validate:"gt=0,gtfield=MinPrice"`
				}

				vl := New[PriceComparison]()
				data := PriceComparison{
					MinPrice: 50.00,
					MaxPrice: 150.00,
				}

				err := vl.Validate(&data)
				assert.NoError(t, err)
			},
			expectErr: false,
		},
		{
			name: "gtfield: string greater than valid",
			testFunc: func(t *testing.T) {
				type StringComparison struct {
					FirstName  string `validate:"required"`
					SecondName string `validate:"required,gtfield=FirstName"`
				}

				vl := New[StringComparison]()
				data := StringComparison{
					FirstName:  "Alice",
					SecondName: "Bob",
				}

				err := vl.Validate(&data)
				assert.NoError(t, err)
			},
			expectErr: false,
		},
		{
			name: "gtfield: negative numbers greater than valid",
			testFunc: func(t *testing.T) {
				type NegativeComparison struct {
					Lower int `validate:"gtfield=Upper"`
					Upper int
				}

				vl := New[NegativeComparison]()
				data := NegativeComparison{
					Lower: -5,
					Upper: -10,
				}

				err := vl.Validate(&data)
				assert.NoError(t, err)
			},
			expectErr: false,
		},

		// gtefield (field greater than or equal to another field) tests
		{
			name: "gtefield: int greater or equal valid greater",
			testFunc: func(t *testing.T) {
				type VersionComparison struct {
					MinVersion int `validate:"gt=0"`
					MaxVersion int `validate:"gt=0,gtefield=MinVersion"`
				}

				vl := New[VersionComparison]()
				data := VersionComparison{
					MinVersion: 1,
					MaxVersion: 2,
				}

				err := vl.Validate(&data)
				assert.NoError(t, err)
			},
			expectErr: false,
		},
		{
			name: "gtefield: int greater or equal valid equal",
			testFunc: func(t *testing.T) {
				type VersionComparison struct {
					MinVersion int `validate:"gt=0"`
					MaxVersion int `validate:"gt=0,gtefield=MinVersion"`
				}

				vl := New[VersionComparison]()
				data := VersionComparison{
					MinVersion: 1,
					MaxVersion: 1,
				}

				err := vl.Validate(&data)
				assert.NoError(t, err)
			},
			expectErr: false,
		},
		{
			name: "gtefield: int greater or equal invalid",
			testFunc: func(t *testing.T) {
				type VersionComparison struct {
					MinVersion int `validate:"gt=0"`
					MaxVersion int `validate:"gt=0,gtefield=MinVersion"`
				}

				vl := New[VersionComparison]()
				data := VersionComparison{
					MinVersion: 2,
					MaxVersion: 1,
				}

				err := vl.Validate(&data)
				require.Error(t, err)
			},
			expectErr: true,
		},
		{
			name: "gtefield: float greater or equal valid equal",
			testFunc: func(t *testing.T) {
				type ScoreComparison struct {
					MinScore float64 `validate:"gte=0"`
					MaxScore float64 `validate:"gte=0,gtefield=MinScore"`
				}

				vl := New[ScoreComparison]()
				data := ScoreComparison{
					MinScore: 75.5,
					MaxScore: 75.5,
				}

				err := vl.Validate(&data)
				assert.NoError(t, err)
			},
			expectErr: false,
		},
		{
			name: "gtefield: string greater or equal valid equal",
			testFunc: func(t *testing.T) {
				type StringComparison struct {
					StartStr string `validate:"required"`
					EndStr   string `validate:"required,gtefield=StartStr"`
				}

				vl := New[StringComparison]()
				data := StringComparison{
					StartStr: "test",
					EndStr:   "test",
				}

				err := vl.Validate(&data)
				assert.NoError(t, err)
			},
			expectErr: false,
		},
		{
			name: "gtefield: zero values greater or equal valid",
			testFunc: func(t *testing.T) {
				type ZeroGreaterOrEqualCheck struct {
					Field1 int `validate:"gtefield=Field2"`
					Field2 int
				}

				vl := New[ZeroGreaterOrEqualCheck]()
				data := ZeroGreaterOrEqualCheck{
					Field1: 0,
					Field2: 0,
				}

				err := vl.Validate(&data)
				assert.NoError(t, err)
			},
			expectErr: false,
		},

		// ltfield (field less than another field) tests
		{
			name: "ltfield: int less than valid",
			testFunc: func(t *testing.T) {
				type AgeRange struct {
					MinAge int `validate:"gt=0,ltfield=MaxAge"`
					MaxAge int `validate:"gt=0"`
				}

				vl := New[AgeRange]()
				data := AgeRange{
					MinAge: 18,
					MaxAge: 65,
				}

				err := vl.Validate(&data)
				assert.NoError(t, err)
			},
			expectErr: false,
		},
		{
			name: "ltfield: int less than invalid equal",
			testFunc: func(t *testing.T) {
				type AgeRangeWithLt struct {
					MinAge int `validate:"gt=0,ltfield=MaxAge"`
					MaxAge int `validate:"gt=0"`
				}

				vl := New[AgeRangeWithLt]()
				data := AgeRangeWithLt{
					MinAge: 30,
					MaxAge: 30,
				}

				err := vl.Validate(&data)
				require.Error(t, err)
			},
			expectErr: true,
		},
		{
			name: "ltfield: float less than valid",
			testFunc: func(t *testing.T) {
				type TemperatureRange struct {
					MinTemp float64 `validate:"ltfield=MaxTemp"`
					MaxTemp float64
				}

				vl := New[TemperatureRange]()
				data := TemperatureRange{
					MinTemp: -10.5,
					MaxTemp: 40.0,
				}

				err := vl.Validate(&data)
				assert.NoError(t, err)
			},
			expectErr: false,
		},
		{
			name: "ltfield: string less than valid",
			testFunc: func(t *testing.T) {
				type StringOrder struct {
					First  string `validate:"required,ltfield=Second"`
					Second string `validate:"required"`
				}

				vl := New[StringOrder]()
				data := StringOrder{
					First:  "Apple",
					Second: "Zebra",
				}

				err := vl.Validate(&data)
				assert.NoError(t, err)
			},
			expectErr: false,
		},
		{
			name: "ltfield: negative numbers less than valid",
			testFunc: func(t *testing.T) {
				type NegativeComparison struct {
					Lower int `validate:"ltfield=Upper"`
					Upper int
				}

				vl := New[NegativeComparison]()
				data := NegativeComparison{
					Lower: -10,
					Upper: -5,
				}

				err := vl.Validate(&data)
				assert.NoError(t, err)
			},
			expectErr: false,
		},

		// ltefield (field less than or equal to another field) tests
		{
			name: "ltefield: int less or equal valid less",
			testFunc: func(t *testing.T) {
				type RankRange struct {
					StartRank int `validate:"gt=0,ltefield=EndRank"`
					EndRank   int `validate:"gt=0"`
				}

				vl := New[RankRange]()
				data := RankRange{
					StartRank: 10,
					EndRank:   20,
				}

				err := vl.Validate(&data)
				assert.NoError(t, err)
			},
			expectErr: false,
		},
		{
			name: "ltefield: int less or equal valid equal",
			testFunc: func(t *testing.T) {
				type RankRange struct {
					StartRank int `validate:"gt=0,ltefield=EndRank"`
					EndRank   int `validate:"gt=0"`
				}

				vl := New[RankRange]()
				data := RankRange{
					StartRank: 15,
					EndRank:   15,
				}

				err := vl.Validate(&data)
				assert.NoError(t, err)
			},
			expectErr: false,
		},
		{
			name: "ltefield: float less or equal valid equal",
			testFunc: func(t *testing.T) {
				type PriceRange struct {
					MinPrice float64 `validate:"gt=0,ltefield=MaxPrice"`
					MaxPrice float64 `validate:"gt=0"`
				}

				vl := New[PriceRange]()
				data := PriceRange{
					MinPrice: 50.00,
					MaxPrice: 50.00,
				}

				err := vl.Validate(&data)
				assert.NoError(t, err)
			},
			expectErr: false,
		},
		{
			name: "ltefield: string less or equal valid equal",
			testFunc: func(t *testing.T) {
				type StringBoundary struct {
					Start string `validate:"required,ltefield=End"`
					End   string `validate:"required"`
				}

				vl := New[StringBoundary]()
				data := StringBoundary{
					Start: "same",
					End:   "same",
				}

				err := vl.Validate(&data)
				assert.NoError(t, err)
			},
			expectErr: false,
		},
		{
			name: "ltefield: zero values less or equal valid",
			testFunc: func(t *testing.T) {
				type ZeroLessOrEqualCheck struct {
					Field1 int `validate:"ltefield=Field2"`
					Field2 int
				}

				vl := New[ZeroLessOrEqualCheck]()
				data := ZeroLessOrEqualCheck{
					Field1: 0,
					Field2: 0,
				}

				err := vl.Validate(&data)
				assert.NoError(t, err)
			},
			expectErr: false,
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			tt.testFunc(t)
		})
	}
}

// ==================================================
// Cross-constraint combinations and edge cases
// ==================================================

// TestCrossFieldComparison tests CrossFieldComparison validation.
func TestCrossFieldComparison(t *testing.T) {
	tests := []struct {
		name      string
		testFunc  func(t *testing.T)
		expectErr bool
	}{
		{
			name: "multiple constraints valid",
			testFunc: func(t *testing.T) {
				type TransactionBounds struct {
					MinAmount    float64 `validate:"gt=0"`
					MaxAmount    float64 `validate:"gt=0,gtfield=MinAmount"`
					ActualAmount float64 `validate:"gt=0,gtefield=MinAmount,ltefield=MaxAmount"`
				}

				vl := New[TransactionBounds]()
				data := TransactionBounds{
					MinAmount:    10.0,
					MaxAmount:    100.0,
					ActualAmount: 50.0,
				}

				err := vl.Validate(&data)
				assert.NoError(t, err, "expected no validation errors for ActualAmount within bounds")
			},
			expectErr: false,
		},
		{
			name: "multiple constraints invalid",
			testFunc: func(t *testing.T) {
				type TransactionBounds struct {
					MinAmount    float64 `validate:"gt=0"`
					MaxAmount    float64 `validate:"gt=0,gtfield=MinAmount"`
					ActualAmount float64 `validate:"gt=0,gtefield=MinAmount,ltefield=MaxAmount"`
				}

				vl := New[TransactionBounds]()
				data := TransactionBounds{
					MinAmount:    10.0,
					MaxAmount:    100.0,
					ActualAmount: 150.0,
				}

				err := vl.Validate(&data)
				require.Error(t, err, "expected validation error for ActualAmount > MaxAmount")
			},
			expectErr: true,
		},
		{
			name: "three way dependency valid",
			testFunc: func(t *testing.T) {
				type OrderValidation struct {
					OrderTotal     float64 `validate:"gt=0"`
					DiscountAmount float64 `validate:"gte=0,ltefield=OrderTotal"`
					FinalAmount    float64 `validate:"gt=0,ltfield=OrderTotal,gtfield=DiscountAmount"`
				}

				vl := New[OrderValidation]()
				data := OrderValidation{
					OrderTotal:     100.0,
					DiscountAmount: 20.0,
					FinalAmount:    80.0,
				}

				err := vl.Validate(&data)
				assert.NoError(t, err, "expected no validation errors for valid order calculation")
			},
			expectErr: false,
		},
		{
			name: "time values if supported",
			testFunc: func(t *testing.T) {
				type EventSchedule struct {
					StartTime time.Time
					EndTime   time.Time
				}

				vl := New[EventSchedule]()
				now := time.Now()
				later := now.Add(2 * time.Hour)

				data := EventSchedule{
					StartTime: now,
					EndTime:   later,
				}

				err := vl.Validate(&data)
				// Test just validates that no panic occurs
				_ = err
			},
			expectErr: false,
		},
		{
			name: "uint comparison valid",
			testFunc: func(t *testing.T) {
				type PortRange struct {
					MinPort uint `validate:"gt=0,ltfield=MaxPort"`
					MaxPort uint `validate:"gt=0"`
				}

				vl := New[PortRange]()
				data := PortRange{
					MinPort: 8000,
					MaxPort: 9000,
				}

				err := vl.Validate(&data)
				assert.NoError(t, err, "expected no validation errors for MinPort < MaxPort")
			},
			expectErr: false,
		},
		{
			name: "uint comparison invalid",
			testFunc: func(t *testing.T) {
				type PortRange struct {
					MinPort uint `validate:"gt=0,ltfield=MaxPort"`
					MaxPort uint `validate:"gt=0"`
				}

				vl := New[PortRange]()
				data := PortRange{
					MinPort: 9000,
					MaxPort: 8000,
				}

				err := vl.Validate(&data)
				require.Error(t, err, "expected validation error for MinPort > MaxPort")
			},
			expectErr: true,
		},
		{
			name: "int32 comparison valid",
			testFunc: func(t *testing.T) {
				type Int32Range struct {
					Start int32 `validate:"ltfield=End"`
					End   int32
				}

				vl := New[Int32Range]()
				data := Int32Range{
					Start: 100,
					End:   200,
				}

				err := vl.Validate(&data)
				assert.NoError(t, err, "expected no validation errors for Start < End")
			},
			expectErr: false,
		},
		{
			name: "float32 comparison valid",
			testFunc: func(t *testing.T) {
				type Float32Range struct {
					MinVal float32 `validate:"ltfield=MaxVal"`
					MaxVal float32
				}

				vl := New[Float32Range]()
				data := Float32Range{
					MinVal: 1.5,
					MaxVal: 2.5,
				}

				err := vl.Validate(&data)
				assert.NoError(t, err, "expected no validation errors for MinVal < MaxVal")
			},
			expectErr: false,
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			tt.testFunc(t)
		})
	}
}
