	...
	formatTests := []formatTestConfig{
		{
			...
			pedantigoTag:   "url",
			expectedErrMsg: "must be a valid URL",
			testCases: []formatTestCase{
				...
				{"Valid FTP scheme", `{"website":"ftp://example.com"}`, false, false, "ftp://example.com", false},
				{"Valid postgres scheme", `{"website":"postgres://localhost:5432/db"}`, false, false, "postgres://localhost:5432/db", false},
				{"Valid file scheme", `{"website":"file:///etc/passwd"}`, false, false, "file:///etc/passwd", false},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if tt.usePointer {
				...
				vl := New[Code]()
				code, err := vl.Unmarshal([]byte(tt.json))
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			case "int":
				...
				product, err := vl.Unmarshal(jsonData)
...
func TestGe(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			vl := New[Product]()
			product, err := vl.Unmarshal(tt.jsonData)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			case "int_valid":
				...
				vl := New[Status]()
				status, err := vl.Unmarshal([]byte(tt.json))
				require.NoError(t, err)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			case "string_valid":
				...
				vl := New[Message]()
				msg, err := vl.Unmarshal([]byte(tt.json))
				require.NoError(t, err)
			...
			case "int_valid":
				...
				vl := New[Config]()
				cfg, err := vl.Unmarshal([]byte(tt.json))
				require.NoError(t, err)
			...
			case "float_valid":
				...
				vl := New[Rate]()
				r, err := vl.Unmarshal([]byte(tt.json))
				require.NoError(t, err)
			...
			case "pointer_valid":
				...
				vl := New[Event]()
				ev, err := vl.Unmarshal([]byte(tt.json))
				require.NoError(t, err)
...
func TestEqBoolValue(t *testing.T) {
	...
	// Valid - exactly true
	result, err := vl.Unmarshal([]byte(`{"enabled":true}`))
	require.NoError(t, err)
	assert.True(t, result.Enabled)

	// Invalid - false when eq=true
	_, err = vl.Unmarshal([]byte(`{"enabled":false}`))
	require.Error(t, err)
	var ve *ValidationError
	require.ErrorAs(t, err, &ve)
	foundError := false
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			case "string_valid":
				...
				vl := New[Status]()
				s, err := vl.Unmarshal([]byte(tt.json))
				require.NoError(t, err)
...
func TestNeWithCombinedConstraints(t *testing.T) {
	...
	t.Run("ne with required", func(t *testing.T) {
		...
		result, err := vl.Unmarshal([]byte(`{"status":"active"}`))
		require.NoError(t, err)
		assert.Equal(t, "active", result.Status)

		// Invalid case - equal to "deleted"
		_, err = vl.Unmarshal([]byte(`{"status":"deleted"}`))
		require.Error(t, err)
